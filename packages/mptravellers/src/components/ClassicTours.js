...
const ClassicMPTours = ({state, actions, libraries}) => {
    ...
    if(data.isReady) {
        ...
        data.items.map( ({id}) => {
            ...
            const singleTour = state.source.classictours[id];
            tours.push(singleTour);
        })
    }

    const pageMPTours = state.source.page[41];
    ...
    return ( 
        ...
        <>
            <ContainerBackgroundTour>
                ...
                <TextBackground>
                    <h1>  Classic Machu Picchu - <span>Guided by Locals</span> </h1>
...
export default connect(ClassicMPTours);
